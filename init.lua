...
local function getAlignment(win, sf)
    ...
    if math.abs(wf.x - sf.x) <= tol then
        return "left"
    end
    if math.abs((wf.x + wf.w) - (sf.x + sf.w)) <= tol then
        return "right"
    end
    return "other"
end
...
-- ctrl+alt+left -> left-aligned, small_seq
-- ctrl+alt+shift+right -> left-aligned, large_seq
--
-- (note) cant use alt+left/right cuz that conflicts with native text shortcuts
